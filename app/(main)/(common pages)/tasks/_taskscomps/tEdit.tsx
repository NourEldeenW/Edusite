...
export default function TEdit({ taskId, access, onSuccess }: TEditProps) {
  ...
  useEffect(() => {
    const fetchTaskData = async () => {
      ...
      try {
        const response = await api.get(`${djangoApi}task/tasks/${taskId}/`, {
          headers: {
        ...
        setTaskContentType(task.task_content_type as "text" | "pdf");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (taskContentType === "pdf") {
        if (removeExistingPdf) {
          // Special value to remove existing PDF
          formData.append("task_pdf", "");
        } else if (taskPdf) {
      ...
      <DialogContent className="max-w-[90vw] lg:max-w-6xl max-h-[90dvh] overflow-y-auto bg-bg-base border-border-card">
        <DialogHeader>
          ...
          <DialogDescription className="text-text-secondary">
            Update task details and availability
          </DialogDescription>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 mt-4">
              {/* Grade Selection (readonly) */}
              <div className="space-y-2">
                <Label className="font-medium text-text-primary">Grade</Label>
                <div className="rounded-lg bg-bg-subtle border border-border-default px-4 py-2.5">
                  {availGrades.find((g) => g.id === gradeId)?.name ||
                    "Unknown Grade"}
                </div>
                          ...
                          <div className="md:col-span-1 space-y-2.5">
                            ...
                            <div className="relative">
                              <select
                                ...
                                {availableCenters.map((center: Center) => (
                ...
                {taskContentType === "text" ? (
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      rows={6}
                      className={
                        getError("task_content") ? "border-red-500" : ""
                      }
                    ...
                    {!removeExistingPdf && (
                      <>
                        <Input
                          ...
                          accept=".pdf"
                          onChange={(e) =>
                            setTaskPdf(e.target.files?.[0] || null)
                          }
                          className={
                            getError("task_pdf") ? "border-red-500" : ""
                          }
                        />
                        <p className="text-sm text-text-secondary">
              ...
              {/* Submission Settings (editable except policy) */}
